 ...
 */
import { PHASE_CONFIG, DEFAULT_AGENT_CAPABILITIES, calculatePriority, DEFAULT_RUNTIME_CONFIG } from '@monarch/shared';
...
export async function compile(input, options = {}) {
    ...
    const manifest = compileToManifest(extractedIntent, enhanced, input.context, opts);
 ...
 */
function injectContext(intent, context, _opts) {
    if (!context) {
        return { intent, injectedContext: null };
    }
    // Add project goals to constraints if relevant
    if (context.goals && context.goals.length > 0) {
        const relevantGoals = context.goals.filter(goal => intent.subject.toLowerCase().includes(goal.toLowerCase().slice(0, 10)) ||
 ...
 * Stage 4: Compile to Runtime Manifest
 */
function compileToManifest(intent, enhanced, context, opts) {
...
function mapDomainToPhase(domain) {
    const mapping = {
        research: 'pred',
        design: 'design',
        code: 'develop',
...
function selectAgent(phase, domain) {
    // Find agent with matching phase and domain
    const candidates = DEFAULT_AGENT_CAPABILITIES.filter(cap => cap.phases.includes(phase));
    // Prefer agent with matching domain
    const domainMatch = candidates.find(cap => cap.domains.includes(domain));
    if (domainMatch) {
        return domainMatch.agentId;
    }
    // Fallback to first candidate
    return (candidates[0]?.agentId || 'planner');
}
function buildSystemPrompt(role, phase, intent, context) {
...
function selectTools(role, domain) {
...
function calculateConfidence(intent, enhanced) {
    let confidence = 1.0;
