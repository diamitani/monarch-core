...
import type { ExtractedIntent, Domain, Attachment } from '@monarch/shared';
...
export function extractIntent(ctx: ExtractionContext): ExtractedIntent {
...
function classifyDomain(prompt: string): Domain {
  ...
  let maxDomain: Domain = 'research'; // default
  let maxScore = 0;
...
function extractPrimaryIntent(prompt: string): string {
...
function extractSubject(prompt: string, attachments?: Attachment[]): string {
...
function inferDesiredOutput(domain: Domain, intent: string, prompt: string): string {
  const outputMap: Record<Domain, string> = {
...
function classifyUrgency(prompt: string): 'immediate' | 'queued' | 'scheduled' {
...
function calculateAmbiguity(
  ...
  if (!subject || subject.split(' ').length > 3) score += 0.2;

  // No constraints means less clarity
  if (constraints.length === 0) score += 0.2;
