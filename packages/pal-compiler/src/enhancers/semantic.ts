...
import type { ExtractedIntent, Domain } from '@monarch/shared';
...
const DOMAIN_ENHANCEMENTS: Record<Domain, {
  verbExpansions: Record<string, string>;
...
export function enhanceIntent(intent: ExtractedIntent): EnhancementResult {
...
function expandVerbs(prompt: string, expansions: Record<string, string>): string {
  let result = prompt;
...
function removeHedging(prompt: string): string {
...
function addBestPractices(prompt: string, domain: Domain): string {
  const practices: Record<Domain, string[]> = {
...
function deriveSuccessCriteria(intent: ExtractedIntent, defaultCriteria: string[]): string[] {
...
function deriveEscalationPolicy(
  intent: ExtractedIntent,
  requiresApproval: boolean
): 'auto-proceed' | 'require-approval' | 'human-in-loop' {
