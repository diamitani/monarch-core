...
export class SessionManager {
   ...
   */
  async create(params: CreateSessionParams): Promise<AgentCoreSession> {
   ...
   */
  async get(sessionId: string): Promise<AgentCoreSession | null> {
   ...
   */
  async listByProject(projectId: string): Promise<AgentCoreSession[]> {
    const result: AgentCoreSession[] = [];
   ...
   */
  async listActiveByUser(userId: string): Promise<AgentCoreSession[]> {
    const result: AgentCoreSession[] = [];
   ...
   */
  async cleanupExpired(maxIdleMs: number = 3600000): Promise<number> {
    const now = Date.now();
    let cleaned = 0;
