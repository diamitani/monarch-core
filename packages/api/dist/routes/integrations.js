...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({
            success: false,
            error: { code: 'LIST_INTEGRATIONS_ERROR', message: error.message }
        });
    }
});
/**
...
router.post('/connect', async (req, res) => {
    ...
    try {
        ...
        logger.error('Connect integration failed', error);
        res.status(500).json({
            success: false,
 ...
 */
router.get('/:integrationId/tools', async (req, res) => {
    const integrationId = req.params.integrationId;
    try {
        ...
        logger.error('List tools failed', error, { integrationId });
        res.status(500).json({
            success: false,
            error: { code: 'LIST_TOOLS_ERROR', message: error.message }
        });
    }
});
